from rest_framework import serializers
from django.contrib.auth.models import User
from .models import *
import re

class UserRegistrationSerializer(serializers.ModelSerializer):
    pass2 = serializers.CharField(style={'input_type': 'password'}, write_only=True)

    class Meta:
        model = User
        fields = ['username', 'email', 'password', 'pass2']
        extra_kwargs = {
            'password': {'write_only': True}
        }

    def validate(self, attrs):
        print("EMAIL CHECK:", attrs['email'])
        print("EXISTS:", User.objects.filter(email=attrs['email']).exists())
        print("ALL USERS:", list(User.objects.values_list('email', flat=True)))
        if attrs['password'] != attrs['pass2']:
            print("unmatch pass")
            raise serializers.ValidationError({'password': 'password fields do not match'})
        if User.objects.filter(email=attrs['email'].strip().lower()).exists():
            raise serializers.ValidationError({"message": "Account with email already exists"})
        return attrs
    
    def create(self, validated_data):
        user = User.objects.create_user(
            username=validated_data['username'],
            email=validated_data['email'],
            password=validated_data['password']
        )
        return user
    
class PreviousQuestionsSerializer(serializers.ModelSerializer):
    class Meta:
        model = PreviousQuestions
        fields = ['id', 'question', 'answer_text', 'isCorrect', 'system_answer']
        
    def clean_question(self, q):
        return re.sub(r'^\d+\s*\.\s*', '', q)
        
    def validate(self, attrs):
        user = self.context['request'].user
        length = PreviousQuestions.objects.filter(user=user).count()
        if length > 100:
            questions = PreviousQuestions.objects.filter(user=user).order_by('id')[:length - 100]
            questions.delete()
            
        attrs['question'] = self.clean_question(attrs['question'])
        return attrs
        

class PreviousLeetcodeSerializer(serializers.ModelSerializer):
    class Meta:
        model = PreviousLeetcode
        fields = ['questions', 'link']